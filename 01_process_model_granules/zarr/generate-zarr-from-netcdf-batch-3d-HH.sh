...
# this script converts 3D HH SASSIE-ECCO model netCDF datasets to zarr and pushes them to the cloud.
...
declare -a arr=("KPPdiffS" "KPPdiffT" "KPPviscA" "PHIHYD" "PHIHYDcR")
...
do
  ...
  do
    ...
    cmd="python create-zarr-from-netcdf-HH.py --var_name $x --files_to_process $sn $en --ec2_scratch_dir /nvme_data${disk} --nc_s3_bucket s3://ecco-processed-data/SASSIE/N1/HH/NETCDF/ --zarr_s3_bucket s3://ecco-processed-data/SASSIE/N1/HH/ZARR/ --keep_local_nc_files --keep_local_zarr_store 1> ${x}_${fsn}_${fen}.log 2> ${x}_${fsn}_${fen}.err.log &"
...
## then write to zarr store with data from netcdfs in batches of 400
## this code does NOT push the local ec2 zarr store to the s3 bucket
  ...
  cmd="aws s3 sync /nvme_data${disk}/zarr_tmp/${x}_AVG_DAILY.ZARR/ s3://ecco-processed-data/SASSIE/N1/HH/ZARR/${x}_AVG_DAILY.ZARR/"
