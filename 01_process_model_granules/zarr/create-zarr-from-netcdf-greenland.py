        ...
        if len(ds[dv].dims) == 5:
            if 'k' in ds[dv].dims:
                if 'i_g' in ds[dv].dims:
                    ds[dv] = ds[dv].chunk({'time':time_chunk, 'face':1, 'k':15, 'j':340, 'i_g':340})
                elif 'j_g' in ds[dv].dims:
                    ds[dv] = ds[dv].chunk({'time':time_chunk, 'face':1, 'k':15, 'j_g':340, 'i':340})
                else:
                    ds[dv] = ds[dv].chunk({'time':time_chunk, 'face':1, 'k':15, 'j':340, 'i':340})
            if 'k_l' in ds[dv].dims:
                ds[dv] = ds[dv].chunk({'time':time_chunk, 'face':1, 'k_l':15, 'j':340, 'i':340})
        else:
            # 2D fields don't need rechunking in space
            if 'i_g' in ds[dv].dims:
                ds[dv] = ds[dv].chunk({'time':time_chunk, 'face':1, 'j':680, 'i_g':680})
            elif 'j_g' in ds[dv].dims:
                ds[dv] = ds[dv].chunk({'time':time_chunk, 'face':1, 'j_g':680, 'i':680})
            else:
                # print(f"... dims 'time' 'j' and 'i' found in {ds[dv].dims}")
                ds[dv] = ds[dv].chunk({'time':time_chunk, 'face':1, 'j':680, 'i':680})
    return ds
...
def reorder_dims(xr_dataset):
    ...
    if 'k' in list(xr_dataset.dims):
        ...
        if 'i_g' in xr_dataset.dims:
            tmp = xr_dataset[["time","face","k","j","i_g","k_u","k_l","k_p1","nv"]]
        elif 'j_g' in xr_dataset.dims:
            tmp = xr_dataset[["time","face","k","j_g","i","k_u","k_l","k_p1","nv"]]
        else:
            tmp = xr_dataset[["time","face","k","j","i","k_u","k_l","k_p1","nv"]]
    # if 2D
    else:
        if 'i_g' in xr_dataset.dims:
            tmp = xr_dataset[["time","face","j","i_g","nv"]]
        elif 'j_g' in xr_dataset.dims:
            tmp = xr_dataset[["time","face","j_g","i","nv"]]
        else:
            tmp = xr_dataset[["time","face","j","i","nv"]]
...
def update_metadata(ds):
    ...
    ds.attrs['product_name'] = 'SASSIE_ECCO_L4_OCN_GREENLAND_AVGDAILY_V1R1'
    ds.attrs['id'] = '10.5067/SASSIE-ECCO-GREENLAND-V1R1'
...
def initialize_zarr_store(var_name, nc_ec2_dir_path, ec2_store):
    ...
    # we have 3 faces, 90 depth layers, and 680x680 i,j dims
    # 3D
    if len(var_ds[var_name].dims) == 5:
        de = dask.array.empty([total_num_days, 3, 90, 680, 680], chunks=[1,1,15,340,340])
        
        # convert empty array into DataArray with proper time index
        if 'i_g' in var_ds[var_name].dims:
            tt = xr.DataArray(de, dims=['time','face','k','j','i_g'], coords={'time':sassie_days})
        elif 'j_g' in var_ds[var_name].dims:
            tt = xr.DataArray(de, dims=['time','face','k','j_g','i'], coords={'time':sassie_days})
        else:
            if 'k_l' in var_ds[var_name].dims:
                tt = xr.DataArray(de, dims=['time','face','k_l','j','i'], coords={'time':sassie_days})
            else:
                tt = xr.DataArray(de, dims=['time','face','k','j','i'], coords={'time':sassie_days})
    
    # 2D
    else:
        de = dask.array.empty([total_num_days, 3, 680, 680], chunks=[1,1,680,680])
        
        # convert empty array into DataArray with proper time index
        if 'i_g' in var_ds[var_name].dims:
            tt = xr.DataArray(de, dims=['time','face','j','i_g'], coords={'time':sassie_days})
        elif 'j_g' in var_ds[var_name].dims:
            tt = xr.DataArray(de, dims=['time','face','j_g','i'], coords={'time':sassie_days})
        else:
            tt = xr.DataArray(de, dims=['time','face','j','i'], coords={'time':sassie_days})
    ...
    for nc_slice in slices:
        ...
        if len(nc_set_tmp) > 0:
            ...
            if np.all(np.diff(days_dt64).astype('int') == 1):
                ...
                # drop unneeded vars
                if len(ds_slice[var_name].dims) == 5:
