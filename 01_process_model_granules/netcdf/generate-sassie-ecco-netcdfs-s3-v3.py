...
def extract_greenland_faces_2D(face_arr):
    """
    Generate array with shape (3,680,680) that includes Greenland from faces 1, 3, and 5
    """
    gl_faces = dict()
    # cutout Greenland with 680x680 dims
    gl_faces[1] = face_arr[1][-680:,:680] # [upper 680 rows, first (left most) 680 columns]
    gl_faces[3] = face_arr[3][-680:,:680]
    gl_faces[5] = face_arr[5][-680:,:680]
    
    # make it an array
    gl_faces_array = np.array([gl_faces[1], gl_faces[3], gl_faces[5]])
    
    return gl_faces_array

def extract_greenland_faces_3D(face_arr):
    """
    Generate array with shape (3,90,680,680) that includes Greenland from faces 1, 3, and 5
    """
    gl_faces = dict()
    # cutout Greenland with 680x680 dims
    gl_faces[1] = face_arr[1][:,-680:,:680] # [upper 680 rows, first (left most) 680 columns]
...
def make_2D_GL_ds(field_GL, model_grid_ds, timestamp, grid_point, da_name):
    ...
    ## create DataArray for c point data
    if grid_point == 'c':
        tmp_da = xr.DataArray([field_GL], dims=['time','face','j','i'],\
                                coords={'time':(('time'), np.array([center_time])),'face':(('face'), np.array(face_array))})
    
    ## create DataArray for u point data
    elif grid_point == 'u':
        tmp_da = xr.DataArray([field_GL], dims=['time','face','j','i_g'],\
                                coords={'time':(('time'), np.array([center_time])),'face':(('face'), np.array(face_array))})
    
    ## create DataArray for v point data
    elif grid_point == 'v':
        tmp_da = xr.DataArray([field_GL], dims=['time','face','j_g','i'],\
                                coords={'time':(('time'), np.array([center_time])),'face':(('face'), np.array(face_array))})
...
def make_3D_GL_ds(field_GL, model_grid_ds, timestamp, grid_point, da_name, k_face='center'):
    ...
    if k_face == 'center':
        ## create DataArray for c point data, center
        if grid_point == 'c':
            tmp_da = xr.DataArray([field_GL], dims=['time','face','k','j','i'],\
                                    coords={'time':(('time'),np.array([center_time])),'face':(('face'), np.array(face_array))})
            tmp_da['k'].attrs['axis']  = 'Z'
            
        ## create DataArray for u point data, center
        elif grid_point == 'u':
            tmp_da = xr.DataArray([field_GL], dims=['time','face','k','j','i_g'],\
                                    coords={'time':(('time'),np.array([center_time])),'face':(('face'), np.array(face_array))})
            tmp_da['k'].attrs['axis']  = 'Z'
            
        ## create DataArray for v point data, center
        elif grid_point == 'v':
            tmp_da = xr.DataArray([field_GL], dims=['time','face','k','j_g','i'],\
                                    coords={'time':(('time'),np.array([center_time])),'face':(('face'), np.array(face_array))})
            tmp_da['k'].attrs['axis']  = 'Z'            
            
    elif k_face == 'top':
        ## create DataArray for c point data, top
        tmp_da = xr.DataArray([field_GL], dims=['time','face','k_l','j','i'],\
                                coords={'time':(('time'),np.array([center_time])),'face':(('face'), np.array(face_array))})
        tmp_da['k_l'].attrs['axis']  = 'Z'
...
def process_2D_variable(data_dir, filename, fldList_index, var_tmp_table, vars_table, sassie_n1_geometry_ds, sassie_n1_geometry_ds_GL):
    ...
    ## cut out Greenland sections from Faces 1, 3, and 5
    var_GL  = extract_greenland_faces_2D(data_faces) 
    ...
    var_GL_ds = make_2D_GL_ds(var_GL, sassie_n1_geometry_ds_GL, timestamp, grid_point=grid_point, da_name=var_name)

    return var_HHv2_ds, var_GL_ds


def process_3D_variable(data_dir, filename, fldList_index, var_tmp_table, vars_table, sassie_n1_geometry_ds, sassie_n1_geometry_ds_GL):
    ...
    ## cut out Greenland sections from Faces 1, 3, and 5
    var_GL  = extract_greenland_faces_3D(data_faces) 
    ...
    var_GL_ds = make_3D_GL_ds(var_GL, sassie_n1_geometry_ds_GL, timestamp, grid_point=grid_point, da_name=var_name, k_face=var_k_face)
...
def modify_metadata(ds, var, var_filename_netcdf):   
    ...
    if 'k' in list(ds.dims):
        ...
        ds.k_p1.attrs['comment'] = 'Includes top of uppermost model tracer cell (k_p1=0) and bottom of lowermost tracer cell (k_p1=91).'
        ds.Zu.attrs['comment'] = 'First element is -1m, the depth of the bottom face of the uppermost tracer grid cell. Last element is the depth of the bottom face of the deepest grid cell. The use of 'u' in the variable name follows the MITgcm convention for naming the bottom face of ocean tracer grid cells.'
    ...
    ds.attrs['time_coverage_start'] = str(ds.time_bnds.values[0][0])[:-10]
    ds.attrs['time_coverage_end'] = str(ds.time_bnds.values[0][1])[:-10]
...
def reorder_dims(xr_dataset):
    ...
    if 'k' in list(xr_dataset.dims):
        if 'face' in list(xr_dataset.dims):
            tmp = xr_dataset[["face","time","k","j","i","k_u","k_l","k_p1","nv"]]
        else:
            tmp = xr_dataset[["time","k","j","i","k_u","k_l","k_p1","nv"]]
    # if 2D
    else:
        if 'face' in list(xr_dataset.dims):
            tmp = xr_dataset[["face","time","j","i","nv"]]
        else:
...
def push_nc_dir_from_ec2(nc_root_dir_ec2, root_dest_s3_name):
    ...
    for subdir_name in all_subdirs:
        if subdir_name != '.ipynb_checkpoints':
            ...
            mybucket_HH = str(root_dest_s3_name) + 'HH/NETCDF/' + str(subdir_name)
            mybucket_GL = str(root_dest_s3_name) + 'GREENLAND/NETCDF/' + str(subdir_name)
            subdir_fullpath = Path(nc_root_dir_ec2 + "/" + subdir_name + "/")
            
            nc_files = list(subdir_fullpath.glob('*.nc'))
            nc_files_HH = list(subdir_fullpath.glob('*HH*.nc'))
            nc_files_GL = list(subdir_fullpath.glob('*GREENLAND*.nc'))
    
            print(f'\n>pushing netcdf files in {subdir_name} to s3 buckets : {mybucket_HH} and {mybucket_GL}')
            print(f'... looking for *.nc files in {subdir_name}')
            print(f'... found {len(nc_files)} total nc files to upload')
            print(f'... found {len(nc_files_HH)} HH nc files to upload')
            print(f'... found {len(nc_files_GL)} Greenland nc files to upload')
    
            if len(nc_files)>0:
                ## upload HH files
                cmd=f"aws s3 cp {subdir_fullpath} {mybucket_HH}/ --recursive --exclude '*' --include '*HH*.nc' --no-progress > /dev/null 2>&1"
                print(f'... aws command: {cmd}\n')
                with suppress_stdout():
                   os.system(cmd)
                    
                ## upload Greenland files
                cmd=f"aws s3 cp {subdir_fullpath} {mybucket_GL}/ --recursive --exclude '*' --include '*GREENLAND*.nc' --no-progress > /dev/null 2>&1"
                print(f'... aws command: {cmd}')
                with suppress_stdout():
                   os.system(cmd)
            else:
...
@time_it
def create_HH_netcdfs(data_filename, data_dir_ec2, nc_root_dir_ec2, metadata_dict, sassie_n1_geometry_ds, sassie_n1_geometry_ds_GL, vars_table, save_nc_to_disk):
    """
    Create netCDF files for all variables in a given file. Produce two datasets: 1) HH and 2) Greenland.
    ...
    Args:
        ...
        sassie_n1_geometry_ds (Dataset): The dataset containing geometry information for the HH field.
        sassie_n1_geometry_ds_GL (Dataset): The dataset containing geometry information for the Greenland data.
    ...
    for i in range(len(vars_in_dataset)):
        ...
        if var_tmp_table['n_dims'].values == '3D':
            ## process dataset
            var_HHv2_ds, var_GL_ds = process_3D_variable(data_dir_ec2, data_filename, fldList_index, var_tmp_table,\
                                                         vars_table, sassie_n1_geometry_ds, sassie_n1_geometry_ds_GL)
            ...
            var_HHv2_ds = mask_dry_grid_cells(var_HHv2_ds, var_name, sassie_n1_geometry_ds, \
                                              grid_point=var_tmp_table['cgrid_point'].values)
            var_GL_ds = mask_dry_grid_cells(var_GL_ds, var_name, sassie_n1_geometry_ds_GL, \
                                              grid_point=var_tmp_table['cgrid_point'].values)
            #print('\nds after 3d masking:')
        ...
        ## 2D data processing 
            ...
            ## process dataset
            var_HHv2_ds, var_GL_ds = process_2D_variable(data_dir_ec2, data_filename, fldList_index, var_tmp_table,\
                                                         vars_table, sassie_n1_geometry_ds, sassie_n1_geometry_ds_GL)
            #print('\nds after 2d loading')
            #show_me_the_ds(var_HHv2_ds)
        ...
        var_GL_ds = ecco.add_global_metadata(global_latlon_metadata, var_GL_ds, var_tmp_table['n_dims'].values[0])
        var_GL_ds = ecco.add_coordinate_metadata(metadata_dict['ECCOv4r4_coordinate_metadata_for_latlon_datasets'], var_GL_ds, less_output=True)
        var_GL_ds, grouping_keywords = ecco.add_variable_metadata(metadata_dict['ECCOv4r4_geometry_metadata_for_latlon_datasets'], var_GL_ds, less_output=True)
        var_GL_ds, grouping_keywords = ecco.add_variable_metadata(metadata_dict['ECCOv4r4_variable_metadata'], var_GL_ds, less_output=True)
        
        ## generate filenames
        ## HH filename
        center_time = var_HHv2_ds.time.values
        yyyy_mm_dd = str(center_time)[2:6] + "-" + str(center_time)[7:9] + "-" + str(center_time)[10:12]
        var_filename_netcdf_HH = var_name + "_day_mean_" + yyyy_mm_dd + "_ECCO_SASSIE_V1_HH_llc1080.nc"
        
        ## Greenland filename
        center_time = var_GL_ds.time.values
        yyyy_mm_dd = str(center_time)[2:6] + "-" + str(center_time)[7:9] + "-" + str(center_time)[10:12]
        var_filename_netcdf_GL = var_name + "_day_mean_" + yyyy_mm_dd + "_ECCO_SASSIE_V1_GREENLAND_llc1080.nc"
        
        ## tweak some of the global attributes
        var_HHv2_ds = modify_metadata(var_HHv2_ds, var_name, var_filename_netcdf_HH)
        var_GL_ds = modify_metadata(var_GL_ds, var_name, var_filename_netcdf_GL)

        ## edit metadata specifically for Greenland dataset:
        var_GL_ds.attrs['summary'] = 'This dataset provides data variable and geometric parameters for the Greenland region of the lat-lon-cap 1080 (llc1080) native model grid from the SASSIE ECCO ocean model Version 1 Release 1 (V1r1) ocean and sea-ice state estimate.'
        var_GL_ds.attrs['title'] = 'SASSIE Ocean Model ' + var_name + ' Parameter for the Greenland region of the Lat-Lon-Cap 1080 (llc1080) Native Model Grid (Version 1 Release 1)'
        var_GL_ds.attrs['comment'] = 'SASSIE llc1080 V1R1 fields are consolidated onto a single curvilinear grid face focusing on the Arctic domain using fields from the 5 faces of the lat-lon-cap 1080 (llc1080) native grid used in the original simulation. This dataset specifically contains the Greenland region of the llc1080 grid comprised of faces 1, 3, and 5.'
        
        ## reorder dims
        var_HHv2_ds = reorder_dims(var_HHv2_ds)
        var_GL_ds = reorder_dims(var_GL_ds)
        
        #data_dir_ec2
        ## save netcdf
        if save_nc_to_disk:
            save_sassie_netcdf_to_ec2(var_HHv2_ds, nc_dir_ec2, var_filename_netcdf_HH)
            save_sassie_netcdf_to_ec2(var_GL_ds, nc_dir_ec2, var_filename_netcdf_GL)
...
def generate_sassie_ecco_netcdfs(root_filenames, root_s3_name, root_dest_s3_name, 
    ...
    ## HH geometry
    n1_geometry_HH_local_filename = 'GRID_GEOMETRY_SASSIE_HH_V1R1_NATIVE_LLC1080.nc'
    n1_geometry_HH_local_full_path = n1_geometry_local_dir / n1_geometry_HH_local_filename
    n1_geometry_HH_s3_url = 's3://ecco-processed-data/SASSIE/N1/HH/GRID/GRID_GEOMETRY_SASSIE_HH_V1R1_NATIVE_LLC1080.nc'
    
    ## Greenland geometry
    n1_geometry_GL_local_filename = 'GRID_GEOMETRY_SASSIE_GREENLAND_V1R1_NATIVE_LLC1080.nc'
    n1_geometry_GL_local_full_path = n1_geometry_local_dir / n1_geometry_GL_local_filename
    n1_geometry_GL_s3_url = 's3://ecco-processed-data/SASSIE/N1/GREENLAND/GRID/GRID_GEOMETRY_SASSIE_GREENLAND_V1R1_NATIVE_LLC1080.nc'

    print('> loading geometry datasets')
    if not n1_geometry_HH_local_full_path.is_file(): # checks if file is there
        ...
        ## HH field
        try:
            s3.download(n1_geometry_HH_s3_url, str(n1_geometry_HH_local_full_path))
            print(f"...downloaded {n1_geometry_HH_s3_url} to {n1_geometry_HH_local_full_path}")
        except:
            print(f"ERROR: could not download {n1_geometry_HH_s3_url} to {n1_geometry_HH_local_full_path}")
            return
    
    if not n1_geometry_GL_local_full_path.is_file(): # checks if file is there
        ## Greenland field
        try:
            s3.download(n1_geometry_GL_s3_url, str(n1_geometry_GL_local_full_path))
            print(f"...downloaded {n1_geometry_GL_s3_url} to {n1_geometry_GL_local_full_path}")
        except:
            print(f"ERROR: could not download {n1_geometry_GL_s3_url} to {n1_geometry_GL_local_full_path}")
            return

    sassie_n1_geometry_ds = xr.open_dataset(n1_geometry_HH_local_full_path)
    sassie_n1_geometry_ds.close()
    print('... HH geometry file loaded')

    sassie_n1_geometry_ds_GL = xr.open_dataset(n1_geometry_GL_local_full_path)
    sassie_n1_geometry_ds_GL.close()
    print('... Greenland geometry file loaded')
    ...
    for data_url in data_urls_select:
        ...
        for data_file in data_files:
            ...
            create_HH_netcdfs(filename, str(gz_dir_ec2), str(nc_root_dir_ec2), metadata_dict, sassie_n1_geometry_ds, sassie_n1_geometry_ds_GL, vars_table, save_nc_to_disk)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--root_dest_s3_name", action="store",
                        help="The destination s3 bucket where processed netcdfs will be stored (e.g., s3://ecco-processed-data/SASSIE/N1/HH/NETCDF/).", 
